...
spage = 1 # 访问起始页
sdays = 1 # 允许抓取天数
baseurl = "http://www.cctime.com/"

urls = ["http://www.cctime.com/list/11084-0++0.htm","http://www.cctime.com/list/9627-0++0.htm"]
...
def level1():
    ...
    try:
         ...
         print( "飞象网")
         print(days)
         text = soup.find_all(name='div', attrs={"class":"kcs_list"})
         if text:
             for item in text:
                 turl = item.find(name='a').get('href')
                 tdate = item.find(name='span').text.split()[0].replace("[","")
                 if tdate in days:
                     tname = item.find(name='a').text
                     #ttname = item.find(name='a').text
                     #try:
                     #    tname = ttname.encode(codetype).decode('gbk').encode('utf8').decode()
                     #except Exception as e:
                     #    tname = ttname
                     if selmysql(tname):
                        continue
                     alltitle.append((turl,))
             eday = text[-1].find(name='span').text.split()[0].replace("[","")
             print("末尾日期为",eday)
             if eday in days:
                 print(days)
                 spage += 1
...
def target(arg,count=1):
    ...
    try:
        ...
        article = soup.find(name='div',attrs={"class":"art_content"})
        if title and article:
            ...
            for item in body:
                ...
                titem = re.sub("</a>","",titem)
                titem = re.sub("(<strong>)?\s*飞象网讯\s*(</strong>)?\s*（.*?）","",titem)
                articles += titem
            #print( articles)
            print()
...
if __name__ == "__main__":
    ...
    #target(('http://www.cctime.com/html/2021-3-3/1561860.htm',))
    print("--  end  --")
    #inmysql('1','1','1','1')
