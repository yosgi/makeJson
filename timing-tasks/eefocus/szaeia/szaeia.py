...
汽车电子: SELECT id FROM `eef_channel_channel` where title='汽车电子';
...
spage = 1 # 访问起始页
sdays = 1 # 允许抓取天数
baseurl = "http://www.szaeia.com/"
url = "http://www.szaeia.com/index.php?m=content&c=index&a=lists&catid=14&page=0++0"
...
def level1(ourl=None,odays=None):
    ...
    try:
         ...
         print( "汽车电子")
         text = soup.find(name='ul', attrs={"class":"venture"})
         if text:
             allmes = text.find_all(name='li')
             for item in allmes:
                 turl = item.find(name='a').get('href')
                 tdate = item.find(name='div', attrs={"class":"col-md-5"}).text
                 tdate = re.sub("\s+(.*)?\n*","",tdate)
                 print("获取到文章时间", tdate)
                 if tdate in ldays:
             ...
             #ldays.pop()
             eday = re.sub("\s+(.*)?\n*","",allmes[-1].find(name='div', attrs={"class":"col-md-5"}).text)
             if eday in ldays:
                 print(ldays)
                 print("末尾日期为",eday)
...
def target(arg,count=1):
    ...
    try:
        ...
        articles = ""
        soup = BeautifulSoup(rdata,"html.parser")
        title = soup.find(name='h1', attrs={"class":"text-center f20 pt20"})
        article = soup.find(name='div', attrs={"id":"content"})
        
        if title and article:
            print( "获取到文章")
            #print(title.text.split("\n")[0])
            name = re.sub(" $","",str(title.text.split("\n")[0]))
            #print( "文章名",name)
            body = str(article)
            titem = re.sub("<a.*?>","",body)
            titem = re.sub("</a>","",titem)
            titem = re.sub("<div.*?>","",titem)
            titem = re.sub("</div>","",titem)
            articles += titem
            #print( articles)
            onlychars = articles
            ...
            #print(articles)
            articles += '<br><span style="box-sizing: border-box; color: rgb(51, 51, 51); letter-spacing: 0px; font-size: 16px; font-family: 微软雅黑; text-align: justify; text-indent: 24px;">来源：汽车电子网</span></p>'
            p = Pool(3)
...
if __name__ == "__main__":
    ...
    for item in data:
        target(item)
    #target(('/index.php?m=content&c=index&a=show&catid=14&id=22764', '2021-02-23'))
    #inmysql('1','1','1','1')
