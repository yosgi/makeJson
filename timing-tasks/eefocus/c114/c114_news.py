...
spage = 1 # 访问起始页
sdays = 1 # 允许抓取天数
baseurl = "http://www.c114.com.cn/"

url = "http://www.c114.com.cn/news/roll.asp"
...
def level1():
    ...
    try:
         ...
         data = {"y":int(year),"m":"%-d" % int(mon),"d":"%-d" % int(day),"searchdate.x":random.randint(1,39),"searchdate.y":random.randint(1,19)}
         print(data)
         ret = requests.post(url,data,verify=False,headers=getheader(),timeout=20)
         codetype = ret.encoding
         #ret.encoding='ISO-8859-1'
         rdata = ret.content
         ...
         print( "C114通信网 新闻")
         text = soup.find(name='div', attrs={"class":"content_c_list"})
         if text:
             allmes = text.find_all(name='div',attrs={"class":"new_list_c"})
             for item in allmes:
                 turl = item.find(name='a')
                 if turl:
                     turl = item.find(name='a').get('href')
                     tname = item.find(name='a').text
                     print()
                     print(codetype)
                     if turl == '/video':
                         print('url获取错误，获取第二项')
                         turl = item.find_all(name='a')[1].get('href')
                         tname = item.find_all(name='a')[1].text
                     #xx=(item.find(name='a').text).encode('ISO-8859-1').decode('gbk').encode('utf8')
                     #print(xx.decode())
                     #print("%r" % xx.decode())
                     #print(item.find(name='a').text.decode('gb2312').encode('utf-8'))
                     #spcode = [u'\u20ac']
                     #for sc in spcode:
                     #    tname = u"%s" %  tname
                     #    tname = tname.replace(sc,"")
                     pd = 0
                     try:
                         print("尝试转码 ",codetype)
                         tname=(tname).encode(codetype).decode('gbk').encode('utf8').decode()
                         pd = 1
                     except Exception as e:
                         try:
                             print("尝试转码 GB2312")
                             tname=(tname).encode('GB2312').decode('gbk').encode('utf8').decode()
                             pd = 1
                         except Exception as e:
                             print("\033[31m 文章命中出现无法解码字符 \033[0m")
                     if not pd:
                         print("转码失败")
                         continue
                     print(turl)
...
def target(arg,count=1):
    ...
    try:
        ...
        articles = ""
        soup = BeautifulSoup(rdata,"html.parser")
        text = soup.find(name="div",attrs={"class":"left-texts"})
        title = text.find(name='h1')
        user = text.find(name='div',attrs={"class":"author"})
        day = text.find(name='div',attrs={"class":"r_time"}).text.split()[0]
        if day not in days:
            print("文章不在时间范围内")
            return
        print(title)
        print(user)
        article = soup.find(name='div', attrs={"id":"text1"})
        if title and article:
            print( "获取到文章")
            try:
                name = title.text.encode(codetype).decode('gbk').encode('utf8').decode()
            except Exception as e:
                name = title.text
            print( "文章名 " , name)
            try:
                user = user.text.encode(codetype).decode('gbk').encode('utf8').decode()
                user = user.split()[1:]
            except Exception as e:
                user = user.text.split()[1:]
            body = article.find_all(name="p")
            print( "文章内容")
            for item in body:
                #print( item)
                try:
                    item = item.encode(codetype).decode('gbk').encode('utf8').decode()
                except Exception as e:
                    pass
                titem = str(item)
                titem = re.sub("<a.*?>","",titem)
                titem = re.sub("</a>","",titem)
                titem = re.sub('<img src="https://www.c114.com.cn/news/images/t21.gif"/>','',titem)
                titem = re.sub('<img src="http://www.c114.com.cn/news/images/t21.gif"/>','',titem)
                articles += titem
            #print( articles)
            print()
            imgs = article.find_all(name="img")
            for img in imgs:
                if img.get('src') in ["https://www.c114.com.cn/news/images/t21.gif","http://www.c114.com.cn/news/images/t21.gif"]:
                    print("c114 图标")
                else:
            ...
            if user:
                try:
                    users = " ".join(user).encode(codetype).decode('gbk').encode('utf8').decode()
                except Exception as e:
                    users = " ".join(user)
                articles += '<br><span style="box-sizing: border-box; color: rgb(51, 51, 51); letter-spacing: 0px; font-size: 16px; font-family: 微软雅黑; text-align: justify; text-indent: 24px;">来源：C114通信网<br>作者：%s</span></p>' % users
            else:
                articles += '<br><span style="box-sizing: border-box; color: rgb(51, 51, 51); letter-spacing: 0px; font-size: 16px; font-family: 微软雅黑; text-align: justify; text-indent: 24px;">来源：C114通信网</span></p>'
            p = Pool(3)
...
if __name__ == "__main__":
    ...
    for item in data:
        target(item)
    #target(('http://www.c114.com.cn/news/127/a1154732.html',))
    print("--  end  --")
    #inmysql('1','1','1','1')
