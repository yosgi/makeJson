...
durls = {"http://www.c114.com.cn/api/ajax/jishu.asp?p=0++0&idn=166&_=++0++":"http://www.c114.com.cn/tech/166.html","http://www.c114.com.cn/api/ajax/jishu.asp?p=0++0&idn=165&_=++0++":"http://www.c114.com.cn/tech/165.html"}
...
def level1():
    ...
    try:
         ...
         print(timestamp)
         lurl = url.replace("0++0",str(spage)).replace("++0++",timestamp)
         print(lurl)
         ...
         print( "C114通信网 新闻")
         text = soup.find_all(name='div', attrs={"class":"news"})
         if text:
        #     endarticle = text.pop()
             for item in text:
         ...
         #    endturl = endarticle.find(name='a').get('href')
         #    endname = endarticle.find(name='a').text
         #    alltitle.append((endturl,))
         #    print("最后文章名",endname)
             #if not selmysql(endname):
             #    spage += 1
             #    alltitle += level1()
             return alltitle
         else:
...
if __name__ == "__main__":
    ...
    for url,referer in durls.items():
        spage = 1
        print(url,referer)
    #print(level1())
        data = level1()
        for item in data:
            target(item)
    #target(('http://www.c114.com.cn/tech/166/a1146864.html',))
    print("--  end  --")
    #inmysql('1','1','1','1')
